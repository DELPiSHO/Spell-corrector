...
sentences = {
    ...
    "How are you?": "Howw are yyu?",
}


def spelltestAutocorrect(speller, tests, verbose=2):
    speller = Speller()
    start = timer()
    n, bad = 0, 0
    for target, incorrect_spellings in tests.items():
        for incorrect_spelling in incorrect_spellings.split("|"):
            n += 1
            w = speller.autocorrect_word(incorrect_spelling)
            if w != target:
                bad += 1
                if verbose >= 2:
                    print(
    ...
    return RESULT.format(bad, n, int(100. - 100. * bad / n), timedelta(seconds=end - start))

def spelltestSpellChecker(spellChecker, tests, verbose=2):
    ...
    for target, incorrect_spellings in tests.items():
        for incorrect_spelling in incorrect_spellings.split("|"):
            n += 1
            w = spellChecker.correction(incorrect_spelling)
            if w != target:
                bad += 1
                if verbose >= 2:
                    print(
...
def sentenceTests(speller, tests, verbose=2):
    speller = Speller()
    start = timer()
    n, bad = 0, 0
    for target, incorrect_spellings in tests.items():
        for incorrect_spelling in incorrect_spellings.split("|"):
            n += 1
            w = speller.autocorrect_sentence(incorrect_spelling)
            if w != target:
                bad += 1
                if verbose >= 2:
                    print(
...
autocorrect = Speller("en")
spellChecker = SpellChecker()

test = "Hey! Mr. Tambourime Man, play a ssong for me,"
good = spellChecker.split_words(test)
print([spellChecker.correction(word) for word in good])

print(spelltestAutocorrect(autocorrect,all_good))
print("\n\n\n")
print(spelltestSpellChecker(spellChecker,all_good))

print(spelltestAutocorrect(autocorrect,BIGTEST))
print("\n\n\n")
print(spelltestSpellChecker(spellChecker,BIGTEST))
print("\n\n\n")

print(sentenceTests(autocorrect,sentences))
